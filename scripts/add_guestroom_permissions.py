...
def main():
    ...
    # Find the position to insert field permissions (before first objectPermissions)
    obj_perm_pos = content.find('    <objectPermissions>')
    if obj_perm_pos == -1:
        print("Could not find objectPermissions in permission set")
        return

    # Insert field permissions before objectPermissions
    new_field_perms = guestroom_type_perms + '\n' + guestroom_type_day_perms + '\n'
    content = content[:obj_perm_pos] + new_field_perms + content[obj_perm_pos:]

    # Find where to insert object permissions (after existing objectPermissions)
    # Find the last </objectPermissions> before tabSettings
    tab_settings_pos = content.find('    <tabSettings>')
    last_obj_perm_end = content.rfind('</objectPermissions>', 0, tab_settings_pos)
    if last_obj_perm_end == -1:
        print("Could not find objectPermissions end tag")
        return

    insert_pos = last_obj_perm_end + len('</objectPermissions>')
    new_obj_perms = '\n' + guestroom_type_obj_perm + '\n' + guestroom_type_day_obj_perm
    content = content[:insert_pos] + new_obj_perms + content[insert_pos:]
