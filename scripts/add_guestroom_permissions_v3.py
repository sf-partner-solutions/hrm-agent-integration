...
Order: fieldPermissions, hasActivationRequired, label, objectPermissions, recordTypeVisibilities, tabSettings, userPermissions
"""
...
def main():
    ...
    # 1. Insert field permissions (before hasActivationRequired)
    ...
    # 2. Insert object permissions (before recordTypeVisibilities)
    record_type_pos = content.find('    <recordTypeVisibilities>')
    if record_type_pos == -1:
        print("Could not find recordTypeVisibilities in permission set")
        return

    new_obj_perms = guestroom_type_obj_perm + '\n' + guestroom_type_day_obj_perm + '\n'
    content = content[:record_type_pos] + new_obj_perms + content[record_type_pos:]

    # 3. Insert tab settings (before userPermissions)
